...
- 未配置任何额度策略时使用平台默认月度 100 积分；未配置模型费率时按下方内置规则记入成本台账。
...
实际积分 = 每次请求积分
         ...
         + billable_units / billable_unit_size × 单位积分单价
```

未显式配置模型费率时，按调用形态应用以下内置规则：

| 调用形态 | 计费单位 | 内置积分 |
| --- | --- | ---: |
| 文本生成输入 | 输入 Token | 0.5 / 1000 Token |
| 文本生成输出 | 输出 Token | 1.5 / 1000 Token |
| 图片理解（Chat 输入包含图片） | 输入图片张数 | 5 / 张，另加文本输入输出 Token 积分 |
| 图片生成 | 输出图片张数 | 250 / 张 |
| TTS | 输入字符数 | 400 / 1000 字符 |
| 文本向量化 | 输入字符总数 | 1 / 1000 字符 |
| 视频生成 | 视频时长 | 1000 / 秒 |

Token 和字符计费均按比例计算，不按千单位向上取整。字符数按 Python Unicode 字符数统计。图片、字符和视频秒数通过带类型的 `billable_units` 参与预占和结算。显式配置的 `ModelCreditRateInput` 会完整覆盖相应模型的内置规则。
