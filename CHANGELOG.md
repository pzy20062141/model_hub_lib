...
- 新增多模态内置积分规则：图片理解 5 积分/张、图片生成 250 积分/张、TTS 400 积分/千字符、文本向量化 1 积分/千字符、视频生成 1000 积分/秒。
- 文本生成内置费率调整为输入 0.5 积分/千 Token、输出 1.5 积分/千 Token；图片理解在图片积分基础上叠加文本 Token 积分。
- Usage 和费率快照增加计费单位类型与单位基数，保证预估 Usage 和供应商 Token Usage 并存时仍按图片、字符或秒数正确结算。
